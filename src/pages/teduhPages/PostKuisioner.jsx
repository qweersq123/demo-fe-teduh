...
import { fetchData, postData, putData } from '../../helpers/api';
...
const PostKuisioner = () => {
        ...
        fetchData('/quest-category/1', config)
            .then((res) => {
                ...
                fetchData('/history-cycle', { params, headers: config.headers })
                    .then((res2) => {
                        const dataJawaban = res2.data.data.find((data) => data.cycle === cycle)?.data?.post;
                        if (dataJawaban) {
    ...
    const handleSubmit = () => {
        ...
        if (hasUnanswered) {
            ...
            if (isDisabledSelect) {
                navigate('/dashboard/terima-kasih', {
                    state: {
    ...
    useEffect(() => {
        console.log(dataAnswerRespondent);
    }, [dataAnswerRespondent]);

    return (
        <div className='w-full flex justify-center'>
            <div className='w-[600px]'>
                <p className='text-2xl mb-5'>Post Kuisioner</p>
